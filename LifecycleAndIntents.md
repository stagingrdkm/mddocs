...
Important to know that your **first frame rendered & committed** to the display is pre-requisite for being able to move to `active` state. Do not wait too long with that because it will directly affect App startup performance.
...
- The application normally becomes visible after the onStateChanged to active but this is not guaranteed. Visibility and focus must be confirmed by the operator app, which is responsible for setting application visibility and granting it focus via Window Manager. This typically happens quasi immediately after onStateChanged to active event.  
...
- Revalidate external sessions, sockets, and handshakes; recreate them when they are expired or the remote side closed or discarded them
- When the socket connection to the Firebolt App GW is no longer active, the applications needs to redo points 2 to 5 of [startup contract](https://github.com/stagingrdkm/mddocs/blob/review_bart/LifecycleAndIntents.md#startup-contract)
